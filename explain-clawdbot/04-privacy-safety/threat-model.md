...
Source: `src/config/types.hooks.ts:11,40` (mapping and gmail types), `src/cron/types.ts:93` (cron payload type), `src/gateway/hooks-mapping.ts:18,53` (hooks resolution)
...
- `isTrustedProxyAddress()` checks if the connecting IP is in your trusted list (`src/gateway/net.ts:136-148`)
- `resolveClientIp()` only reads `X-Forwarded-For`/`X-Real-IP` headers when the immediate connection comes from a trusted proxy (`src/gateway/net.ts:151-180`)
- `isLocalDirectRequest()` uses both checks to determine if a request is genuinely local (`src/gateway/auth.ts:115`)
...
Source: `src/gateway/auth.ts:104` (`resolveTailscaleClientIp()`), `src/gateway/auth.ts:115` (`isLocalDirectRequest()`), `src/security/audit.ts` (audit check)
...
- Subagent exposure is limited: `filterBootstrapFilesForSession()` (`src/agents/workspace.ts:557-565`) restricts subagents to only AGENTS.md + TOOLS.md
...
Source: `src/agents/sandbox/config.ts:224` (`resolveSandboxConfigForAgent()`)
...
Source: `src/agents/sandbox/tool-policy.ts:208` (`resolveSandboxToolPolicyForAgent()`)
