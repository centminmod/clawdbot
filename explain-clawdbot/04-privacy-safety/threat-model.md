...
- Subagent exposure is limited: `filterBootstrapFilesForSession()` (`src/agents/workspace.ts:336-344`) restricts subagents to only AGENTS.md + TOOLS.md

Source: `src/agents/workspace.ts:30-31` (file list), `src/agents/pi-embedded-helpers/bootstrap.ts:84,162-191` (injection), `src/memory/qmd-manager.ts:346-353` (QMD validation)
