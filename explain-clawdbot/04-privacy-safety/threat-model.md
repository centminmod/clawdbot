...
- `isLocalDirectRequest()` uses both checks to determine if a request is genuinely local (`src/gateway/auth.ts:125`)
...
Source: `src/gateway/auth.ts:97` (`resolveTailscaleClientIp()`), `src/gateway/auth.ts:125` (`isLocalDirectRequest()`), `src/security/audit.ts` (audit check)
...
- Subagent exposure is limited: `filterBootstrapFilesForSession()` (`src/agents/workspace.ts:542-550`) restricts subagents to only AGENTS.md + TOOLS.md
