...
- Misinterpreted `disable-model-invocation: true` in the skill frontmatter as meaning the skill was "disabled" (it actually controls whether the model can invoke the skill unprompted — parsed at `src/agents/skills/frontmatter.ts:220`, used at `src/agents/skills/workspace.ts:544`)
...
# Source: src/config/types.tools.ts:233
