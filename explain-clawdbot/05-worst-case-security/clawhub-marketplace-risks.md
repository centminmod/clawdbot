...
Integration: `src/agents/skills-install.ts:115-131` (install-time scan), `src/security/audit-extra.async.ts:1149-1325` (security audit deep scan)
...
| Install-time scan integration | `src/agents/skills-install.ts:115-131` | Collects scan warnings during skill install |
| Security audit deep scan | `src/security/audit-extra.async.ts:1149-1325` | Plugin code safety findings for `--deep` audit |
...
- [Prompt Injection Attacks](./prompt-injection-attacks.md) - 30 attack examples; skills can embed injection payloads in .md and .mmd files
