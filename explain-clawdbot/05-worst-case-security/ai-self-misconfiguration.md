...
Source: `src/config/io.ts:343-362` — `rotateConfigBackups()` keeps `.bak` files before each write.
...
|-----------|-------------|--------|
| **Config backup rotation** | Keeps 5 `.bak` files before each config write | `src/config/io.ts:343-362` |
