...
OpenClaw uses Zod schemas with `.strict()` mode (`src/config/zod-schema.ts:902`). This means:
...
| **Strict schema validation** | Zod `.strict()` rejects unknown top-level keys and type errors | `src/config/zod-schema.ts:902` |
