...
1. **File permissions** (`src/infra/fs-safe.ts:41-42`) — files opened with `O_NOFOLLOW` on non-Windows platforms, blocking symlink-based exfiltration
2. **Symlink protection** (`src/infra/fs-safe.ts:64-65, 72-73`) — double-checked via `isSymlinkOpenError()` catch and explicit `lstat().isSymbolicLink()` check
3. **Path traversal prevention** (`src/pairing/pairing-store.ts:53-62`) — `safeChannelKey()` strips `../`, `/`, `\`, and other path traversal characters
4. **Root escape prevention** (`src/infra/fs-safe.ts:107-125`) — `openFileWithinRoot()` verifies resolved path stays within root directory
