...
2. **Environment variable blocklist** (`src/agents/bash-tools.exec-runtime.ts:40-54`) — dangerous env vars (`LD_PRELOAD`, `DYLD_INSERT_LIBRARIES`, `NODE_OPTIONS`, `BASH_ENV`, etc.) are blocked via `isDangerousHostEnvVarName()` (policy in `src/infra/host-env-security-policy.json`), plus prefix-based blocking for `DYLD_*` and `LD_*`
3. **Sanitization enforcement** (`src/agents/bash-tools.exec-runtime.ts:57-76`) — `validateHostEnv()` throws `Security Violation` errors if blocked vars or PATH modifications are detected
...
6. **SSRF policy enforcement** (`src/infra/net/ssrf.ts:292-363`) — `resolvePinnedHostnameWithPolicy()` + `resolvePinnedHostname()` block both hostname-based and resolved-IP-based private network access
...
**Subagent mitigation:** `filterBootstrapFilesForSession()` at `src/agents/workspace.ts:551-559` limits subagents to only `AGENTS.md` + `TOOLS.md`, reducing the bootstrap attack surface from 9 files to 2 in multi-agent setups.
