...
1. **`d637a2635`** (PR [#15592](https://github.com/openclaw/openclaw/pull/15592)) — **Gateway: sanitize WebSocket log headers:** Added `sanitizeLogValue()` in `src/gateway/server/ws-connection.ts:21-41` which removes control characters (U+0000-U+001F, U+007F-U+009F), Unicode format characters (Cf category), collapses whitespace, and truncates to 300 chars max using `truncateUtf16Safe()`. Applied to `forwardedFor`, `requestOrigin`, `requestHost`, `requestUserAgent`, and close `reason` before logging. Prevents log injection via crafted headers and control character sequences in WebSocket connection logs.
