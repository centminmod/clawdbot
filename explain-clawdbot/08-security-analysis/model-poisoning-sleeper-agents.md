...
- Code ref: `src/agents/models-config.providers.ts:768-778` — Ollama configured as HTTP endpoint
...
- Code ref: `src/memory/embeddings.ts:78-79` — default model `hf:ggml-org/embeddinggemma-300m-qat-q8_0-GGUF`
...
| Embedding model default | `src/memory/embeddings.ts:78-79` | Auto-download from HuggingFace (QAT variant) |
| Embedding providers | `src/memory/embeddings.ts:39` | openai/local/gemini/voyage options |
| API model providers | `src/agents/models-config.providers.ts` | All API provider configs |
| Ollama HTTP endpoint | `src/agents/models-config.providers.ts:768-778` | `http://127.0.0.1:11434/v1` |
