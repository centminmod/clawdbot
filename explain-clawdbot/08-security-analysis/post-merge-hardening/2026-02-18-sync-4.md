...
|-----|---------|-------|-------------|
| `4b40bdb98` | fix(telegram): clear offsets on token change | 1 prod + 1 test + 1 doc | Clears Telegram update offsets when bot tokens change. `channelsAddCommand` at `src/commands/channels/add.ts:159-177` compares previous and new tokens, calls `deleteTelegramUpdateOffset()` at line 230. Prevents stale offset state after token rotation. |
