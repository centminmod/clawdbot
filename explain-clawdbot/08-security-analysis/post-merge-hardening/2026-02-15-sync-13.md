...
10. **`b79e7fdb7` + `edb06170f` + `6863b9dbe`** — **fix(image): workspace/sandbox media allowlist:** Extended image tool `localRoots` in `src/agents/tools/image-tool.ts:4621-4639` to include workspace dir (after resolving and rejecting filesystem roots) and added state workspace/sandbox dirs to `getDefaultLocalRoots()` in `src/web/media.ts:8612-8621`. **Related to Legitimate Gap #3 (outPath validation).**
