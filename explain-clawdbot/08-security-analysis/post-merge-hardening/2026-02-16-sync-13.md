...
| `c1655982d` | refactor(telegram): extract group-allow and thread helpers | 2 prod + 1 test | Refactor |
| `09566b169` | refactor(discord): extract channel-id resolution helper | 2 prod + 1 test | Refactor |
| `ddcc7a1a5` | fix(discord): simplify channel ID to string coercion | 1 prod | Refactor |
...
5. **`adc818db4`** — **fix(gateway): serve Control UI bootstrap config and lock down CSP**
   ...
   - JSON endpoint at lines 246-274 serves bootstrap config as `application/json` with `no-cache`
...
|-----|---------|-------|--------|
| `c1655982d` | refactor(telegram): extract group-allow and thread helpers | 2 prod + 1 test | Extracts `resolveTelegramGroupAllowFromContext()` and shifts `resolveTelegramForumThreadId()` to `helpers.ts:69`. No auth/security boundary changes. |
| `09566b169` | refactor(discord): extract channel-id resolution helper | 2 prod + 1 test | Extracts `resolveDiscordMessageChannelId()`. No security boundary changes. |
| `ddcc7a1a5` | fix(discord): simplify channel ID to string coercion | 1 prod | Simplifies BigInt→string coercion. No security changes. |
