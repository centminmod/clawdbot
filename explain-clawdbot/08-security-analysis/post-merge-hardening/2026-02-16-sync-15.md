...
| `d1fca442b` | refactor(sandbox): centralize sha256 helpers | 3 prod | **Helper Extraction:** Added `hashTextSha256()` at hash.ts:7 and updated `slugifySessionKey()` at shared.ts:7 to use centralized SHA-256. Related to `559c8d993` hardening. |
...
| `41ded303b` | fix(sandbox): preserve array order in config hashing | 1 prod + 1 test + 1 doc | **Config Hash Consistency:** Changed `normalizeForHash()` at config-hash.ts:22-33 to preserve primitive array order instead of sorting. Ensures DNS/capability bind mounts produce deterministic container hashes. |
