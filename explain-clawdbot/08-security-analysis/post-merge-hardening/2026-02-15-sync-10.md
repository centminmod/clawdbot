...
4. **2a3da2133** — **Session key normalization for send policy** (`src/sessions/send-policy.ts:23-91`): New `stripAgentSessionKeyPrefix()` function (lines 23-33) handles canonical `agent:<agentId>:<sessionKey>` format. `deriveChannelFromKey()` (line 35) and `deriveChatTypeFromKey()` (line 47) updated to normalize keys. `resolveSendPolicy()` (lines 61-91) checks both raw and stripped keys against prefix rules, preventing format-dependent policy bypasses.

5. **ee8d8be2e** — **OAuth manual code flow with validation** (`src/agents/chutes-oauth.ts:36-68`): `parseOAuthCallbackInput()` now accepts bare authorization codes via manual input. Validates: non-empty after trim (line 41), attempts URL parse first (line 46), falls back to bare code only if no whitespace and no `://` scheme (line 62). Maintains state parameter pass-through for CSRF protection. Improves OAuth usability for environments where browser redirect fails.
