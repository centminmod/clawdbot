...
|-----|---------|-------------------|-------|
| 166cf6a3e | fix(web_fetch): cap response body before parsing | **Yes** — DoS prevention | 7 files (5 production + 2 test) |
...
**Files:**
- `src/agents/tools/web-fetch-utils.ts` (NEW) — 250 lines
...
2. **Response body capping** before JSON parsing in `web-shared.ts`:
   - New `MAX_RESPONSE_BODY_BYTES` constants (configurable, with safe defaults)
   - Early abort if response exceeds limits, preventing memory exhaustion
   - Fallback to truncated/partial responses rather than crashing
