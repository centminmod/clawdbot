...
2. **5e7c3250c** — **Workspace-only path guards for filesystem tools** (14 files): Adds `workspaceOnly` parameter to `createApplyPatchTool()` at `src/agents/apply-patch.ts:47-49`. `resolvePatchPath()` (lines 254-285) gains a `purpose` parameter; calls `assertSandboxPath()` at line 274 for workspace-only enforcement. New `wrapToolWorkspaceRootGuard()` at `src/agents/pi-tools.read.ts:255` wraps read/write/list tools with workspace root validation. Applied to all tools via `pi-tools.ts:283,297,308`. Addresses **Audit 1 Claims #5 (file permission checks), #6 (path traversal)**, and partially mitigates **Gap #3 (outPath validation)**.
