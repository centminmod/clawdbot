...
| `99d099aa8` | Gateway: add APNs push test pipeline (#20307) | 11 prod + 2 test + 1 doc | Adds `push.test` RPC endpoint and CLI command for APNs connectivity testing. New handler at `push.ts:19` validates params via JSON schema, loads device registration, and sends visible alert via `sendApnsAlert()` at `push-apns.ts:406`. Device tokens validated with regex `/^[0-9a-f]{32,}$/i` via `isLikelyApnsToken()` at :154 before storage. Registered as WRITE_METHOD at `server-methods.ts:99` requiring authentication. Environment override restricted to "sandbox" or "production" only. |
