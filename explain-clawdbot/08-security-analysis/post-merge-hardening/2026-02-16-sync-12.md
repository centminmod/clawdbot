...
|-----|---------|-------|----------|
| `887b209db` | fix(security): harden sandbox docker config validation | 11 prod + 3 test | Security |
...
1. **`887b209db`** — **fix(security): harden sandbox docker config validation**
   ...
   New `validateSandboxSecurity()` function (`src/agents/sandbox/validate-sandbox-security.ts:198`) enforces runtime security checks when creating sandbox containers:
   ...
   - Blocks dangerous security profiles: `seccompProfile: "unconfined"` and `apparmorProfile: "unconfined"` rejected at both Zod schema level (`src/config/zod-schema.agent-runtime.ts:128-155`) and runtime
   - Path traversal protection: `normalizeHostPath()` (line 55) resolves `..` sequences and `//` normalization
   - Symlink escape blocking: `validateBindMounts()` (line 124) uses `realpathSync()` to resolve symlinks before checking against denylist
   - Ancestor mount detection: `getBlockedBindReasonStringOnly()` (line 71) detects ancestor mounts (e.g., `/run:/run` exposes Docker socket)
...
2. **`6254e96ac`** — **fix(security): harden prompt path sanitization (OC-19)**
   ...
   - Applied to workspaceDir at `src/agents/system-prompt.ts:359`
