...
1. **`887b209db`** — **fix(security): harden sandbox docker config validation**
   ...
   - Path traversal protection: `normalizeHostPath()` (line 52) resolves `..` sequences and `//` normalization
   - Symlink escape blocking: `validateBindMounts()` (line 122) uses `realpathSync()` to resolve symlinks before checking against denylist
   - Ancestor mount detection: `getBlockedBindReasonStringOnly()` (line 64) detects ancestor mounts (e.g., `/run:/run` exposes Docker socket)
