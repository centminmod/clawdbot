...
1. **Audit 2, Claim 5** (Self-approving agent/no RBAC): Commit `7c822d039` introduces plugin hooks that can observe all LLM I/O without agent approval. While hooks are "fire-and-forget" (cannot modify behavior), they create a new surveillance surface that bypasses agent visibility. Operators relying on agent-level approval controls should note that plugins can independently observe all prompts and responses.
