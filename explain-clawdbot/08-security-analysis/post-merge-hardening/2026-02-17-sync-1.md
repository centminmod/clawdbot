...
1. **`095d52209`** — **NEW** Session transcript files created with 0o600 permissions. Modified `ensureSessionHeader()` at `src/config/sessions/transcript.ts:77` and transcript file creation at `src/gateway/server-methods/chat.ts:124` to enforce owner-read/write-only mode. Also updated `src/security/fix.ts` with permission fix logic for existing `.jsonl` transcript files.

2. **`553d17f8a`** — **REINFORCES** Silent token constant usage. Replaced hardcoded "NO_REPLY" strings with `SILENT_REPLY_TOKEN` constant at `src/agents/subagent-announce.ts:368-370` and `src/agents/system-prompt.ts:115,124,604` to prevent accidental disclosure of internal control tokens.

3. **`68e39cf2c`** — Hardens QR pairing flow validation. Added check at `src/cli/qr-cli.ts:101-103` to error when `--remote` flag is used without `gateway.remote.url` or `gateway.tailscale.mode=serve/funnel` configured.

4. **`df6d0ee92`** — Refactors tool policy logic. Created `pickSandboxToolPolicy()` at new file `src/agents/sandbox-tool-policy.ts:21`, consolidating duplicate security-critical code previously scattered across `src/agents/pi-tools.policy.ts:213-216,289` and `src/security/audit-tool-policy.ts`.

5. **`f4b2fd00b`** — Hardens config merge-by-id fallback. Enhanced `mergeObjectArraysById()` at `src/config/merge-patch.ts:24-78` to append patch entries without IDs instead of rejecting, preventing data loss during config operations.

6. **`b4fa10ae6`** — Infrastructure hardening for fetch wrapper. Added idempotency marker `wrapFetchWithAbortSignalMarker` at `src/infra/fetch.ts:9,37,93` to prevent double-wrapping which could bypass SSRF protections.

7. **`e3e8046a9`** — Stability fix for fetch wrapper error handling. Restructured try/catch/finally at `src/infra/fetch.ts:45-68` to prevent detached finally unhandled rejections that could mask security-relevant errors.

8. **`cb391f4bd`** — Data integrity fix for config.patch. Relaxed requirement at `src/config/merge-patch.ts:17-33` that ALL patch entries need IDs, preventing array destruction when patch entries lack IDs.
