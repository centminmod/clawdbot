...
|-----|---------|-------|----------|
| `beb77229c` | fix (security/line): fail closed when webhook auth is missing | 2 prod + 1 test | Security |
...
| `2363e1b08` | fix(security): restrict skill download target paths | 5 prod + 4 test | Security |
| `c6c53437f` | fix(security): scope session tools and webhook secret fallback | 16 prod + 5 test + 5 doc | Security |
| `d95be2c38` | fix: preserve sandbox allow-all semantics | 2 prod + 1 test | Security |
| `6957354d4` | fix (telegram/whatsapp): use account-scoped pairing allowlists | 5 prod + 1 test | Security |
...
1. **`beb77229c`** — **fix (security/line): fail closed when webhook auth is missing**
   ...
   - `extensions/line/src/channel.ts:119-127` — `validateWebhookConfig()` now throws explicit errors when either credential is missing
   - `extensions/line/src/channel.ts:603-627` — channel initialization validates both token and secret before proceeding
   - `src/line/monitor.ts:129+` — monitor layer propagates auth validation errors
...
3. **`ee10feb80`** — **fix (security/pairing): scope pairing stores by account**
   ...
   - `src/pairing/pairing-store.ts:63+` — new `resolveCredentialsDir()` includes account ID in path
   - `src/pairing/pairing-store.ts:12-53` — account-scoped storage for pending requests and allowlist entries
...
5. **`c6c53437f`** — **fix(security): scope session tools and webhook secret fallback**
   ...
   - `src/agents/tools/sessions-access.ts:1-240` — new access control module with visibility checks
   - `src/agents/tools/sessions-helpers.ts:56+` — `sessions_visibility` config option (self/tree/agent/all)
   - `src/config/types.tools.ts:17+` — visibility configuration schema
   - Sandboxed sessions default to "tree" visibility (self + spawned only)
...
6. **`d95be2c38`** — **fix: preserve sandbox allow-all semantics**
   ...
   - `src/agents/sandbox/tool-policy.ts:89-92` — checks `expandedAllow.length > 0` before injecting image tool
...
7. **`6957354d4`** — **fix (telegram/whatsapp): use account-scoped pairing allowlists**
   ...
   - `src/telegram/bot-handlers.ts` — multiple handler updates for `accountId`
   - `src/telegram/bot-message-context.ts:231+` — `accountId` added to pairing request
   - `src/web/inbound/access-control.ts` — account-scoped lookup
...
| (67 others) | Tests, perf improvements, Discord UI, memory fixes | ~140 prod + ~75 test | No security boundary changes. |
