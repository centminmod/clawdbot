...
| `356d61aac` | fix(gateway): scope tailscale tokenless auth to websocket | 5 prod + 2 test + 9 doc | `authorizeGatewayConnect()` at `auth.ts:428` gains `allowTailscaleHeaderAuth?: boolean` at `:438`. NEW `resolveTailscaleClientIp()` at `auth.ts:112`. `isLocalDirectRequest()` shifted to `auth.ts:123`. Tailscale tokenless header auth scoped to WebSocket connections only. Partially addresses Audit 1 Claim 7. |
...
- `src/gateway/auth.ts:112`: `resolveTailscaleClientIp()` (NEW function, was `:64-85`)
- `src/gateway/auth.ts:123`: `isLocalDirectRequest()` (was at `:87`)
- `src/gateway/auth.ts:428`: `authorizeGatewayConnect()`
