...
3. **`a45778238`** — Parameter validation hardening: Explicit type checking for `agentId`/`name` in gateway RPC (`src/gateway/server-methods/agents.ts:71-86`) prevents `Object.prototype.toString()` coercion bypass.
...
7. **`0931a3570`** — Null pointer guard: Guards `withSessionStoreLock()` against undefined `storePath` (`src/config/sessions/store.ts:775-780`) preventing crashes during session store initialization failure.
...
15. **`2fe16af3c`** — Agent file path resolution consolidation: Centralizes path validation for agent workspace files (`src/gateway/server-methods/agents.ts:64-93`). Addresses Audit 1 Claim 6 (path traversal).
