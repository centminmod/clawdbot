...
- `src/agents/cli-credentials.ts:388-389` — verified: `try { existingResult = execFileSyncImpl(` (keychain read)
- `src/agents/cli-credentials.ts:408-409` — verified: `const newValue = JSON.stringify(existingData)` (keychain write prep)
