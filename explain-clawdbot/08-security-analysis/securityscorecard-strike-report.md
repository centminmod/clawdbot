...
| **Symlink privilege escalation** | NOT VULNERABLE | `src/infra/fs-safe.ts:48-49` opens files with `O_NOFOLLOW` flag. Line 97 checks `isSymlinkOpenError()`. Line 109 checks `isSymbolicLink()`. Line 118 verifies file identity via `sameFileIdentity()`. Added in post-merge hardening (commit `5eee991`). |
