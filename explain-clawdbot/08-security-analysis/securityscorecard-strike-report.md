...
| **Symlink privilege escalation** | NOT VULNERABLE | `src/infra/fs-safe.ts:43-44` opens files with `O_NOFOLLOW` flag. Line 76 checks `isSymlinkOpenError()`. Line 83 checks `isSymbolicLink()`. Line 93 verifies file identity via `sameFileIdentity()`. Added in post-merge hardening (commit `5eee991`). |
