...
| **SSRF via webhook bypass** | NOT EXPLOITABLE | `extensions/voice-call/src/webhook-security.ts:462,480` has a `skipVerification` parameter, but it is explicitly a dev-only feature. The production path enforces Twilio signature verification with `crypto.timingSafeEqual()`. Previously audited in [Issue #1796](./issue-1796-argus-audit.md). |
| **Symlink privilege escalation** | NOT VULNERABLE | `src/infra/fs-safe.ts:51-52` opens files with `O_NOFOLLOW` flag. Line 108 checks `isSymlinkOpenError()`. Line 120 checks `isSymbolicLink()`. Line 129 verifies file identity via `sameFileIdentity()`. Added in post-merge hardening (commit `5eee991`). |
