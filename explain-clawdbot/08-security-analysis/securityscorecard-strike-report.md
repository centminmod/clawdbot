...
| **Symlink privilege escalation** | NOT VULNERABLE | `src/infra/fs-safe.ts:41-42` opens files with `O_NOFOLLOW` flag. Line 64 checks `isSymlinkOpenError()`. Line 72 checks `isSymbolicLink()`. Line 81 verifies file identity via `sameFileIdentity()`. Added in post-merge hardening (commit `5eee991`). |
