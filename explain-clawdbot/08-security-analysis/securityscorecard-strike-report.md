...
| **Symlink privilege escalation** | NOT VULNERABLE | `src/infra/fs-safe.ts:53-54` opens files with `O_NOFOLLOW` flag. Line 108 checks `isSymlinkOpenError()`. Line 120 checks `isSymbolicLink()`. Line 129 verifies file identity via `sameFileIdentity()`. Added in post-merge hardening (commit `5eee991`). |
