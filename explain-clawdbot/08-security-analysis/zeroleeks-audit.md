...
| Indirect email injection | HTML comment in email body | Direct user message | `buildSafeExternalPrompt()` wraps hook content with security warnings (`src/cron/isolated-agent/run.ts:389`, defined at `src/security/external-content.ts:303`) | **Inconclusive** - bypassed defense layer |
...
| Cron/hook wrapping | `src/cron/isolated-agent/run.ts:389` | External hooks wrapped via `buildSafeExternalPrompt()` (defined at `src/security/external-content.ts:303`) with suspicious pattern logging |
