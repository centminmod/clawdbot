...
5. Is actively integrated into web search (`src/agents/tools/web-search.ts:1196,1223,1251,1277`), web fetch (`src/agents/tools/web-fetch.ts:250-271`), cron hooks (`src/cron/isolated-agent/run.ts:346-381`), Discord (`src/discord/monitor/message-handler.process.ts:197-203`), and Slack (`src/slack/monitor/room-context.ts:15-19`)
...
| Discord metadata isolation | `src/discord/monitor/message-handler.process.ts:197-203` | Channel topics wrapped via `buildUntrustedChannelMetadata()` |
