...
|---|-------------------|---------------------|-----------|
| 1 | `buildSkillsSection` logic (skill scanning, JIT loading) | `src/agents/system-prompt.ts:17-39` | Yes - verbatim match |
| 2 | `buildMemorySection` (memory recall protocol) | `src/agents/system-prompt.ts:41-67` | Yes - verbatim match |
| 3 | `buildReplyTagsSection` (reply tag syntax) | `src/agents/system-prompt.ts:83-96` | Yes - verbatim match |
...
| Indirect email injection | HTML comment in email body | Direct user message | `buildSafeExternalPrompt()` wraps hook content with security warnings (`src/cron/isolated-agent/run.ts:334-340`) | **Inconclusive** - bypassed defense layer |
...
5. Is actively integrated into web search (`src/agents/tools/web-search.ts:1076,1103,1131,1157`), web fetch (`src/agents/tools/web-fetch.ts:250-271`), cron hooks (`src/cron/isolated-agent/run.ts:311-346`), Discord (`src/discord/monitor/message-handler.process.ts:176-182`), and Slack (`src/slack/monitor/message-handler/prepare.ts:455-461`)
...
| Web search wrapping | `src/agents/tools/web-search.ts:1076,1103,1131,1157` | All search snippets wrapped via `wrapWebContent()` |
| Web fetch wrapping | `src/agents/tools/web-fetch.ts:250-271` | All fetched content wrapped with security warnings |
| Cron/hook wrapping | `src/cron/isolated-agent/run.ts:311-346` | External hooks wrapped via `buildSafeExternalPrompt()` with suspicious pattern logging |
| Discord metadata isolation | `src/discord/monitor/message-handler.process.ts:176-182` | Channel topics wrapped via `buildUntrustedChannelMetadata()` |
