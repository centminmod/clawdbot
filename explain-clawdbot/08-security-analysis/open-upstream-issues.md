...
| [#11900](https://github.com/openclaw/openclaw/issues/11900) | ~~MEDIUM~~ WONTFIX | Context files (USER.md, SOUL.md) loaded for all senders | Closed upstream as NOT_PLANNED (2026-03-25); `src/agents/bootstrap-files.ts:64-96` — no `senderIsOwner` check; `attempt.ts:369` calls unconditionally |
...
- `src/agents/pi-embedded-runner/run/attempt.ts:369` — calls `resolveBootstrapContextForRun()` without `senderIsOwner`
- `src/agents/pi-embedded-runner/run/attempt.ts:444` — `senderIsOwner` only passed to `createOpenClawCodingTools()` for tool gating
...
- `src/agents/pi-embedded-runner/run/attempt.ts:428` — `execOverrides` passed to tool creation, but populated only from directives/session
...
- `src/agents/pi-embedded-runner/run/attempt.ts:375` — `contextMode: params.bootstrapContextMode` — `bootstrapContextMode` is not set for subagent sessions; defaults to `undefined`
