...
| [#8586](https://github.com/openclaw/openclaw/issues/8586) | ~~HIGH~~ FIXED | Configurable bypass allows unrestricted command exec | Fixed upstream (COMPLETED 2026-02-14); `src/agents/bash-tools.exec.ts:587-606,666` |
| [#8591](https://github.com/openclaw/openclaw/issues/8591) | HIGH (WONTFIX) | Env vars exposed via shell commands | Closed upstream as NOT_PLANNED (2026-02-13); still affects local code at `src/agents/bash-tools.exec.ts:699,705` |
...
| [#9792](https://github.com/openclaw/openclaw/issues/9792) | INVALID (CLOSED) | validateHostEnv skips baseEnv (by design) | Closed upstream as COMPLETED (2026-03-01); `src/agents/bash-tools.exec-runtime.ts:84` + `src/agents/bash-tools.exec.ts:705`; by-design behavior confirmed |
...
| [#11900](https://github.com/openclaw/openclaw/issues/11900) | ~~MEDIUM~~ WONTFIX | Context files (USER.md, SOUL.md) loaded for all senders | Closed upstream as NOT_PLANNED (2026-03-25); `src/agents/bootstrap-files.ts:64-96` — no `senderIsOwner` check; `attempt.ts:373` calls unconditionally |
...
| [#14875](https://github.com/openclaw/openclaw/issues/14875) | ~~HIGH~~ FIXED | Feishu channel hardcodes CommandAuthorized bypassing access groups | Fixed upstream (COMPLETED 2026-02-13); `extensions/feishu/src/bot.ts:932` |
...
**Affected code:**
- `src/gateway/server-http.ts:904-936` - Canvas/A2UI handler dispatch (now auth-wrapped via `authorizeCanvasRequest()` at `src/gateway/server/http-auth.ts:57`, PR #9518)
- `src/gateway/server-http.ts:1040-1062` - WebSocket upgrade for canvas (`attachGatewayUpgradeHandler`; canvas WS auth-wrapped via `authorizeCanvasRequest()` at `server-http.ts:837`, PR #9518)
...
**Fix:** Query token extraction removed entirely from `src/gateway/hooks.ts`. `extractHookToken()` now only accepts `Authorization: Bearer` header and `X-OpenClaw-Token` header. Server returns HTTP 400 when `?token=` is present (`src/gateway/server-http.ts:495-499`).
...
- `src/infra/device-pairing.ts:679` - device token verification uses `verifyPairingToken()` (wraps `safeEqualSecret()`, fixed in Feb 13 sync 4, commit `113ebfd6a`)
...
- `src/gateway/server-methods/sessions.ts:1188` - `fs.writeFileSync(filePath, ..., "utf-8")` (no mode)
...
**Affected code:**
- `src/agents/bash-tools.exec.ts:587-606` — `elevatedMode` resolution; `elevatedMode=full` sets security to "full", ask to "off"
- `src/agents/bash-tools.exec.ts:666` — `bypassApprovals` flag; skips all approval checks when `elevatedMode === "full"`
...
**Affected code:**
- `src/agents/bash-tools.exec.ts:699,705` — full `process.env` passed to child spawn via `coerceEnv(process.env)` + `overrides: params.env`
- `src/infra/host-env-security-policy.json` + `sanitizeHostExecEnvWithDiagnostics()` at `src/infra/host-env-security.ts:181` — policy blocks injection INTO env (enforcement at `bash-tools.exec.ts:705`), but doesn't filter what children can READ
...
**Vulnerability:** Code correctly uses `mode: 0o700` for directory creation (`src/config/io.ts:2158`), but when installed via `sudo`, the directory inherits root ownership. Subsequent user-space operations may create group-writable files.
...
**Fix:** Query token acceptance completely removed. `extractHookToken()` in `src/gateway/hooks.ts:138-155` no longer reads `url.searchParams`. `src/commands/dashboard.ts` no longer constructs `?token=` URLs. `src/commands/onboard-helpers.ts` no longer passes token in URL. Server now returns HTTP 400 when `?token=` is present (`src/gateway/server-http.ts:495-499`).
...
**Affected code:**
- `src/config/io.ts:2080` - `writeConfigFile` now preserves env var references for unchanged paths (commit `f59df9589`), but resolved values still leak for paths that change
...
**Affected code:**
- `src/config/io.ts:2080` - `writeConfigFile` now preserves env var references for unchanged paths (commit `f59df9589`), partially mitigating meta writeback expansion
...
**Affected code:**
- `src/agents/bash-tools.exec-runtime.ts:84` (definition) + `src/agents/bash-tools.exec.ts:705` (call) — validation scoped to `params.env` only; centralized as `sanitizeHostExecEnv()` at `src/infra/host-env-security.ts:224` in Feb 21 sync 7
...
- `src/agents/pi-embedded-runner/run/attempt.ts:373` — calls `resolveBootstrapContextForRun()` without `senderIsOwner`
- `src/agents/pi-embedded-runner/run/attempt.ts:449` — `senderIsOwner` only passed to `createOpenClawCodingTools()` for tool gating
...
- `src/agents/pi-embedded-runner/run/attempt.ts:433` — `execOverrides` passed to tool creation, but populated only from directives/session
...
- `src/gateway/server-http.ts:509-528` — hook auth failure rate limiting added (Feb 13 sync 4, commit `113ebfd6a`): 20 failures/60s per client IP, HTTP 429 response
...
- `extensions/feishu/src/bot.ts:932` — now uses `CommandAuthorized: commandAuthorized` (dynamic, post-fix)
...
- `src/gateway/server/ws-connection/message-handler.ts:530` — `handleMissingDeviceIdentity()`: device pairing requirement skipped when `allowInsecureAuth` configured
...
- `src/agents/pi-embedded-runner/run/attempt.ts:379` — `contextMode: params.bootstrapContextMode` — `bootstrapContextMode` is not set for subagent sessions; defaults to `undefined`
