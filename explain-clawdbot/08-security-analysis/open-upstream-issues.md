...
| [#10331](https://github.com/openclaw/openclaw/issues/10331) | ~~MEDIUM~~ FIXED | Session store stale cache inside write lock | Fixed upstream (COMPLETED 2026-02-14); `src/config/sessions/store.ts:782,748` |
...
| [#11900](https://github.com/openclaw/openclaw/issues/11900) | ~~MEDIUM~~ WONTFIX | Context files (USER.md, SOUL.md) loaded for all senders | Closed upstream as NOT_PLANNED (2026-03-25); `src/agents/bootstrap-files.ts:64-96` — no `senderIsOwner` check; `attempt.ts:1717` calls unconditionally |
...
| [#54737](https://github.com/openclaw/openclaw/issues/54737) | MEDIUM | Prompt injection via unsanitized group chat metadata (GroupSubject, GroupMembers) | `src/auto-reply/reply/groups.ts:153-165` — `buildGroupChatContext()` embeds `GroupSubject` and `GroupMembers` directly in system prompt string without `sanitizeForPromptLiteral()`; attackers who control a group name (e.g., Discord/Telegram group with malicious title) can inject arbitrary LLM instructions |
...
| [#13683](https://github.com/openclaw/openclaw/issues/13683) | ~~HIGH~~ FIXED | CLI `config get` returns unredacted secrets to sandboxed agents | Fixed upstream (COMPLETED 2026-02-14); `src/cli/config-cli.ts:1137-1139` |
...
**Affected code:**
- `src/gateway/server-http.ts:858-867` - Canvas/A2UI handler dispatch (now auth-wrapped via `authorizeCanvasRequest()` at `src/gateway/server/http-auth.ts:57`, PR #9518)
...
- `src/infra/device-pairing.ts:648` - device token verification uses `verifyPairingToken()` (wraps `safeEqualSecret()`, fixed in Feb 13 sync 4, commit `113ebfd6a`)
...
**Affected code:**
- `src/config/sessions/store.ts:782` — `updateSessionStoreEntry()` inside `withSessionStoreLock()` calls `loadSessionStore(storePath)` **without `skipCache: true`**
- `src/config/sessions/store.ts:748` — `withSessionStoreLock()` helper
- `src/config/sessions/store.ts:568` — `updateSessionStore()` NOW correctly uses `{ skipCache: true }` (bug partially fixed in session pruning refactor)
...
- `src/agents/pi-embedded-runner/run/attempt.ts:1717` — calls `resolveBootstrapContextForRun()` without `senderIsOwner`
- `src/agents/pi-embedded-runner/run/attempt.ts:1771` — `senderIsOwner` only passed to `createOpenClawCodingTools()` for tool gating
...
- `src/agents/pi-embedded-runner/run/attempt.ts:1774` — `execOverrides` passed to tool creation, but populated only from directives/session
...
**Affected code:**
- `src/cli/config-cli.ts:1137-1139` — `loadValidConfig()` returns resolved `snapshot.config`; `getAtPath()` reads values directly
- `src/cli/config-cli.ts:1137-1159` — output paths (`defaultRuntime.log`) emit unredacted values
...
- `src/gateway/server/ws-connection/message-handler.ts:516` — `handleMissingDeviceIdentity()`: device pairing requirement skipped when `allowInsecureAuth` configured
...
- `src/agents/pi-embedded-runner/run/attempt.ts:1723` — `contextMode: params.bootstrapContextMode` — `bootstrapContextMode` is not set for subagent sessions; defaults to `undefined`
